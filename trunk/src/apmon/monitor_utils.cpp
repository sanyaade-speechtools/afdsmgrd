...
void apmon_mon_utils::readJobDiskUsage(MonitoredJob job, 
  ...
  if (retScan != 1) {
    ...
    sprintf(msg, "[ readJobDiskUsage() ] Error reading du output file for process %ld", job.pid);
    throw runtime_error(msg);
  }
  ...
  if (retScan != 5) {	 
    fclose(fp); unlink(du_f);
    sprintf(msg, "[ readJobDiskUsage() ] Error reading df output file for process %ld", job.pid);
    throw runtime_error(msg);
  }
