...
throw(runtime_error) {
  ...
  if (firstTime) {
	...
	if(ioctl(sockd, SIOCGIFADDR, &ifr)<0){
	  sprintf(logmsg, "Cannot get the address of %s", this->interfaceNames[i]);
...
int ApMon::sendTimedParameters(char *clusterName, char *nodeName,
  ...
  int i;
  int ret, ret1, ret2;
  char msg[200], buf2[MAX_HEADER_LENGTH+4], newBuf[MAX_DGRAM_SIZE], crtAddr[20];
  char *headerTmp;
...
DWORD WINAPI bkTask(void *param) {
  ...
  while (1) {
    ...
    /*
    crtTime = time(NULL);
    sprintf(logmsg, "### 2 crt %ld recheck %ld sys %ld ",crtTime,  nextRecheck, 
        nextSysInfoSend);
    logger(FINE, logmsg);
    */
...
void ApMon::setGenMonitoring(bool bGenMonitoring, int nIntervals) {
  ...
  sprintf(logmsg, "Setting general information monitoring to %s ", 
	 boolStrings[(int)bGenMonitoring]);
  logger(INFO, logmsg);
...
void ApMon::initSocket() throw(runtime_error) {
  ...
  if (ret1!=0){
    sprintf(logmsg, "[ initSocket() ] cannot set reuseaddr: %d", ret1);
    logger(WARNING, logmsg); 
  }
  ...
  if (ret2!=0){
    sprintf(logmsg, "[ initSocket() ] cannot set send timeout to %d seconds: %d", optval2.tv_sec, ret2);
    logger(WARNING, logmsg);
  }
  ...
  if (ret3!=0){
    sprintf(logmsg, "[ initSocket() ] cannot set receive timeout to %d seconds: %d", optval2.tv_sec, ret3);
    logger(WARNING, logmsg);
  }
#endif
